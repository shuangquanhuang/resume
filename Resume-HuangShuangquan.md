...
Audio annotation platform is an web platform to manage template and task for taggers to tag audios.
...
Also I extracted some highly reusable components from normal business development, such as a Questionnaire Component, which is used in some other similar project successfully. The design principles are: clear boundary (only focus on questionnaire), extensible JSON schema, plugin and interface pattern for extension, provide HOOKs and utility functions for user to easily customize.It’s designed to have to main Component: Builder and Render, with Builder user can create question template schema, and then use Render to render questions. Both Components ares highly extensible, for example: in Builder you can register customized operators, register your own question type, etc; in Render, question can be render based on DAG conditions, you can register your own single question render component if it follow the interface specification. It’s also exported a StateManager to get the state of current question status, for example, if this question visible or disabled based on the answer of other questions.
...
#### Data analysis on Office 365    2015.09-2016.05
